class ZCL_ZCDOC_OBJ_PI_CHDO definition
  public
  create public .
...
public section.
  ...
  class-data OBJECTCLASS type IF_CHDO_OBJECT_TOOLS_REL=>TY_CDOBJECTCL read-only value 'ZCDOC_OBJ_PI' ##NO_TEXT.
  ...
  class-methods WRITE
    importing
      ...
      !O_ZTB_PI_CC type ZTB_PI_CC optional
      !N_ZTB_PI_CC type ZTB_PI_CC optional
      !UPD_ZTB_PI_CC type IF_CHDO_OBJECT_TOOLS_REL=>TY_CDCHNGINDH default SPACE
    exporting
...
CLASS ZCL_ZCDOC_OBJ_PI_CHDO IMPLEMENTATION.


  method WRITE.
*"----------------------------------------------------------------------
*"         this WRITE method is generated for object ZCDOC_OBJ_PI
*"         never change it manually, please!        :06/27/2025
...
*"----------------------------------------------------------------------
    ...
    CALL METHOD cl_chdo_write_tools=>changedocument_open
     ...
     IF ( N_ZTB_PI_CC IS INITIAL ) AND
        ( O_ZTB_PI_CC IS INITIAL ).
       l_upd  = space.
     ELSE.
       l_upd = UPD_ZTB_PI_CC.
     ENDIF.
     ...
     IF  l_upd  NE space.
       CALL METHOD CL_CHDO_WRITE_TOOLS=>changedocument_single_case
         EXPORTING
           tablename              = 'ZTB_PI_CC'
           workarea_old           = O_ZTB_PI_CC
           workarea_new           = N_ZTB_PI_CC
           change_indicator       = UPD_ZTB_PI_CC
           docu_delete            = ''
           docu_insert            = ''
           docu_delete_if         = ''
           docu_insert_if         = ''
